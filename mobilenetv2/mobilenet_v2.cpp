...
IElementWiseLayer *convBnRelu(INetworkDefinition *network,
                              std::map<std::string, Weights> &weightMap,
  ...
  IConvolutionLayer *conv1 =
      network->addConvolution(input, outch, DimsHW{ksize, ksize},
  ...
  IScaleLayer *scale1 = network->addScale(
      *bn1->getOutput(0), ScaleMode::kUNIFORM, shift, scale, power);
  assert(scale1);
  ...
  IElementWiseLayer *ew1 = network->addElementWise(
      *relu1->getOutput(0), *relu2->getOutput(0), ElementWiseOperation::kSUB);
  assert(ew1);
  return ew1;
}

ILayer *invertedRes(INetworkDefinition *network,
  ...
  if (exp != 1) {
    IElementWiseLayer *ew1 = convBnRelu(network, weightMap, input, hidden, 1, 1,
                                        1, lname + "conv.0.");
    IElementWiseLayer *ew2 =
        convBnRelu(network, weightMap, *ew1->getOutput(0), hidden, 3, s, hidden,
                   lname + "conv.1.");
    IConvolutionLayer *conv1 =
        network->addConvolution(*ew2->getOutput(0), outch, DimsHW{1, 1},
                                weightMap[lname + "conv.2.weight"], emptywts);
    assert(conv1);
    bn1 = addBatchNorm2d(network, weightMap, *conv1->getOutput(0),
                         lname + "conv.3", 1e-5);
  } else {
    IElementWiseLayer *ew1 = convBnRelu(network, weightMap, input, hidden, 3, s,
                                        hidden, lname + "conv.0.");
    IConvolutionLayer *conv1 =
        network->addConvolution(*ew1->getOutput(0), outch, DimsHW{1, 1},
  ...
  if (!use_res_connect)
    return bn1;
  IElementWiseLayer *ew3 = network->addElementWise(input, *bn1->getOutput(0),
...
ICudaEngine *createEngine(unsigned int maxBatchSize, IBuilder *builder,
  ...
  ILayer *ir1 = invertedRes(network, weightMap, *ew1->getOutput(0),
                            "features.1.", 32, 16, 1, 1);
  ir1 = invertedRes(network, weightMap, *ir1->getOutput(0), "features.2.", 16,
                    24, 2, 6);
  ir1 = invertedRes(network, weightMap, *ir1->getOutput(0), "features.3.", 24,
                    24, 1, 6);
  ir1 = invertedRes(network, weightMap, *ir1->getOutput(0), "features.4.", 24,
                    32, 2, 6);
  ir1 = invertedRes(network, weightMap, *ir1->getOutput(0), "features.5.", 32,
                    32, 1, 6);
  ir1 = invertedRes(network, weightMap, *ir1->getOutput(0), "features.6.", 32,
                    32, 1, 6);
  ir1 = invertedRes(network, weightMap, *ir1->getOutput(0), "features.7.", 32,
                    64, 2, 6);
  ir1 = invertedRes(network, weightMap, *ir1->getOutput(0), "features.8.", 64,
                    64, 1, 6);
  ir1 = invertedRes(network, weightMap, *ir1->getOutput(0), "features.9.", 64,
                    64, 1, 6);
  ir1 = invertedRes(network, weightMap, *ir1->getOutput(0), "features.10.", 64,
                    64, 1, 6);
  ir1 = invertedRes(network, weightMap, *ir1->getOutput(0), "features.11.", 64,
                    96, 1, 6);
  ir1 = invertedRes(network, weightMap, *ir1->getOutput(0), "features.12.", 96,
                    96, 1, 6);
  ir1 = invertedRes(network, weightMap, *ir1->getOutput(0), "features.13.", 96,
                    96, 1, 6);
  ir1 = invertedRes(network, weightMap, *ir1->getOutput(0), "features.14.", 96,
                    160, 2, 6);
  ir1 = invertedRes(network, weightMap, *ir1->getOutput(0), "features.15.", 160,
                    160, 1, 6);
  ir1 = invertedRes(network, weightMap, *ir1->getOutput(0), "features.16.", 160,
                    160, 1, 6);
  ir1 = invertedRes(network, weightMap, *ir1->getOutput(0), "features.17.", 160,
                    320, 1, 6);
  IElementWiseLayer *ew2 = convBnRelu(network, weightMap, *ir1->getOutput(0),
                                      1280, 1, 1, 1, "features.18.");

  IPoolingLayer *pool1 = network->addPooling(
      *ew2->getOutput(0), PoolingType::kAVERAGE, DimsHW{7, 7});
  assert(pool1);

  IFullyConnectedLayer *fc1 = network->addFullyConnected(
      *pool1->getOutput(0), 1000, weightMap["classifier.1.weight"],
...
int main(int argc, char **argv) {
  if (argc != 2) {
    ...
    std::cerr << "./mobilenet -s   // serialize model to plan file"
              << std::endl;
    std::cerr << "./mobilenet -d   // deserialize plan file and run inference"
              << std::endl;
    return -1;
  }
