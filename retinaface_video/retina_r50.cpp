...
static const int INPUT_H =
    decodeplugin::INPUT_H; // H, W must be able to  be divided by 32.
...
cv::VideoCapture initCamera(cv::Mat &frame, int fps = 5, int cam_id = 0) {

  // cv::VideoCapture cap(cam_id);
  cv::VideoCapture cap("../test_kevin.webm");

  cap.set(cv::CAP_PROP_FRAME_WIDTH, 640);
  cap.set(cv::CAP_PROP_FRAME_HEIGHT, 480);
  // cap.set(cv::CAP_PROP_FORMAT, CV_32FC4);
  // cap.set(cv::CAP_PROP_FPS, fps);

  std::cout << "Camera stream initialized" << std::endl;
...
void nms(std::vector<decodeplugin::Detection> &res, float *output,
  ...
  for (size_t m = 0; m < dets.size(); ++m) {
    auto &item = dets[m];
    res.push_back(item);
    // std::cout << item.class_confidence << " bbox " << item.bbox[0] << ", " <<
    // item.bbox[1] << ", " << item.bbox[2] << ", " << item.bbox[3] <<
    // std::endl;
...
IActivationLayer *bottleneck(INetworkDefinition *network,
  ...
  IConvolutionLayer *conv3 =
      network->addConvolution(*relu2->getOutput(0), outch * 4, DimsHW{1, 1},
  ...
  if (stride != 1 || inch != outch * 4) {
    IConvolutionLayer *conv4 = network->addConvolution(
...
int main(int argc, char **argv) {
  ...
  ICudaEngine *engine =
      runtime->deserializeCudaEngine(trtModelStream, size, &pf);
  // ICudaEngine* engine = runtime->deserializeCudaEngine(trtModelStream,
  // size, nullptr);
  assert(engine != nullptr);
  ...
  while (cv::waitKey(1) < 0) {
    ...
    for (size_t j = 0; j < res.size(); j++) {
      ...
      // cv::putText(img, std::to_string((int)(res[j].class_confidence * 100)) +
      // "%", cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2,
      // cv::Scalar(0xFF, 0xFF, 0xFF), 1);
      for (int k = 0; k < 10; k += 2) {
        cv::circle(
            img, cv::Point(res[j].landmark[k], res[j].landmark[k + 1]), 1,
            cv::Scalar(255 * (k > 2), 255 * (k > 0 && k < 8), 255 * (k < 6)),
            4);
      }
    }
